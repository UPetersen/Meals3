...
final class FoodDetails {
    ...
    lazy var sections: [SectionVM] = {
        var theSections:[SectionVM] = [SectionVM]()
        ...
        if let rows = rows {
            theSections.append(SectionVM(title: "Allgemeine Informationen", footer: " ", rows: rows))
        }
...
//        print("amount string \(amountString)")
    ...
    func sectionForKeys(_ keys:[String], title:String, footer:String) -> SectionVM {
        ...
        return SectionVM(title: title, footer: footer, rows: rows)
    }
