...
protocol IngredientCollection: NSManagedObject {
    ...
    // Since a protocol cannot state conformance to ObservableObject (Error ist "Protocol 'NutrientCollection' can only be used as a generic constraint because it has Self or associated type requirements", but mea l and recipe already conform to ObservableObject, I have added the property 'objectWillChange'. Thus one can publish when a change of the NutrientCollection (aka Meal or Recipe) occured and achieve similar behavior.
